...
For files that contain only PHP code, the closing tag (`?>`) should not be included. It is not required by PHP. Leaving this out prevents trailing white space from being accidentally injected into the output that can introduce errors in the ITDC session (see the PHP manual on [Instruction separation](http://php.net/basic-syntax.instruction-separation)).
...
Adhering to object oriented programming practice as supported by PHP 5.3+ is required. ITDC projects is committed to progressively making the source code E_STRICT.
...
These "DocBlocks" borrow from the PEAR standard but have some variations specific for ITDC code.
...
If the `@package` tag is used, it will be "ITDCMS".

If the `@subpackage` tag is used, it is the name of the top level folder under the /libraries/ folder. For example: Application, Database, Html, and so on.
...
-   `@subpackage` (optional)
-   `@author` (optional but only permitted in non-ITDC source files, for example, included third-party libraries like PHPMailer)
-   `@copyright` (required)
-   `@license` (required and must be compatible with the MIT license)
-   `@deprecated` (optional)
-   `@link` (optional)
-   `@see` (optional)
...
Functions and methods should be named using the "studly caps" style (also referred to as "bumpy case" or "camel caps"). The initial letter of the name is lowercase, and each letter that starts a new "word" is capitalized.
