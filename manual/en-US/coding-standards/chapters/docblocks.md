...
These "DocBlocks" borrow from the PEAR standard but have some variations specific for ITDC code and the ITDC projects.
...
Code contributed to the ITDC project that will become the copyright of the project is not allowed to include @author tags. You should update the contribution log in CREDITS.php. ITDC's philosophy is that the code is written "all together" and there is no notion of any one person "owning" any section of code. The @author tags are permitted in third-party libraries that are included in the core libraries.
...
```php
/**
 * Controller class to initialise authorization.
 *
 * @package     ITDCMS.Installation
